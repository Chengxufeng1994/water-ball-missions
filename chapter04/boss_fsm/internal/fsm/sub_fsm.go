...
func (fsm *SubFiniteStateMachine) OnEvent(ctx shared.Context, event Event) {
	fsm.HandleEvent(ctx, event)
}
...
// GenerateMessage implements State.
func (fsm *SubFiniteStateMachine) GenerateMessage(event Event) string {
	return fsm.BaseFiniteStateMachine.GetCurrentState().GenerateMessage(event)
}
