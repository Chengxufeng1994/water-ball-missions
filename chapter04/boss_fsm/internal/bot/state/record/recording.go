...
func NewRecording(entryAction, exitAction fsm.Action) *Recording {
	return &Recording{
		BaseState: state.NewBaseState("recording", entryAction, exitAction),
	}
}
...
func (r *Recording) OnEvent(ctx shared.Context, event fsm.Event) {
	if event.GetEventType() == fsm.SpeakEvent {
		payload, _ := event.GetEventPayload().(fsm.SpeakPayload)
