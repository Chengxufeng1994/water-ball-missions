...
func (game *AdventureGame) MonstersTurn() {
	for i := range game.Monsters {
		monster := game.Monsters[i]
		monsterX, monsterY := monster.Position()

		// 怪物攻擊角色
		if monster.InAttackRange(game.Character) {
			monster.Attack(game.Character)
			fmt.Println("Monster attacked Character")
			continue
		}
		...
		for {
			...
			newX, newY := monsterX+direction[0], monsterY+direction[1]
			if err := game.IsMoveAllowed(newX, newY); err != nil {
				fmt.Printf("[ERROR] Monster move not allowed, reason: %s\n", err.Error())
				continue
			}

			monster.Move(newX, newY)
			game.GameMap[newX][newY] = monster
			game.RemoveMapObject(monsterX, monsterY)
			fmt.Printf("Monster from %d, %d moved to %d, %d\n", monsterX, monsterY, newX, newY)
			break
		}
	}
}
...
func (game *AdventureGame) EndGame() bool {
